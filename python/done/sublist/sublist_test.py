...
class SublistTest(unittest.TestCase):
    ...
    def test_non_empty_list_contains_empty_list(self):
        self.assertEqual(check_lists([1, 2, 3], []), SUPERLIST)
        #
    def test_list_equals_itself(self):
        self.assertEqual(check_lists([1, 2, 3], [1, 2, 3]), EQUAL)
        
    def test_different_lists(self):
        self.assertEqual(check_lists([1, 2, 3], [2, 3, 4]), UNEQUAL)
        
    def test_false_start(self):
        self.assertEqual(check_lists([1, 2, 5], [0, 1, 2, 3, 1, 2, 5, 6]), SUBLIST)
        
    def test_consecutive(self):
        self.assertEqual(check_lists([1, 1, 2], [0, 1, 1, 1, 2, 1, 2]), SUBLIST)
        #
    def test_sublist_at_start(self):
        self.assertEqual(check_lists([0, 1, 2], [0, 1, 2, 3, 4, 5]), SUBLIST)
        #
